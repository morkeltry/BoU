...
struct RuntimeState {
    ...
    contract6: contracts::secretnote::SecretNote,
...
lazy_static! {
    static ref STATE: SgxMutex<RuntimeState> = {
        SgxMutex::new(RuntimeState {
            ...
            contract6: contracts::secretnote::SecretNote::new(),
    ...
    let res = match opaque_query.contract_id {
        ...
        SECRET_NOTE => serde_json::to_value(
            state.contract6.handle_query(
                accid_origin.as_ref(),
                types::deopaque_query(opaque_query)
                    .map_err(|_| error_msg("Malformed request (secretnote::Request)"))?.request)
        ).unwrap(),
